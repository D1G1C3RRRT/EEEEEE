...
import { loadLocalBlueprint } from "@/lib/blueprint/storage";

type Props = {
  history: BlueprintSummary[];
  ...
  function runCompare() {
    ...
    try {
      const left =
        ...
        loadLocalBlueprint(leftId);
      const right =
        (current && current.id === rightId ? current : null) ||
        loadLocalBlueprint(rightId);
      if (!left || !right) {
        setError("Oba blueprinty musia byť v histórii (local). Otvor ich skenom alebo importom.");
        return;
      }
  ...
  return (
    <div className="panel p-4 sm:p-5 space-y-3">
      ...
      <label className="block space-y-1">
        <span className="text-[11px] uppercase tracking-wide text-fg-subtle">A (left)</span>
        <select
      ...
      <label className="block space-y-1">
        <span className="text-[11px] uppercase tracking-wide text-fg-subtle">B (right)</span>
        <select
          className="flex h-10 w-full rounded-[var(--radius-sm)] border border-border bg-input px-2 text-sm text-fg"
          value={rightId}
      ...
      <Button
        ...
        onClick={runCompare}
      >
        {busy ? <Loader2 className="size-3.5 animate-spin" /> : <GitCompareArrows className="size-3.5" />}
        Porovnať
      </Button>
        ...
        <div className="space-y-2 rounded-[var(--radius-md)] border border-border bg-bg-subtle/60 p-3">
          <div className="flex flex-wrap gap-1.5">
            <Badge variant={result.identical ? "success" : "warning"}>
              {result.identical ? "identické" : "rozdiely"}
            </Badge>
          ...
          <ul className="text-xs text-fg-muted space-y-1">
            <li>Tech +{result.summary.techAdded.length} / −{result.summary.techRemoved.length}</li>
            <li>Assety Δ {result.summary.assetCountDelta}</li>
            <li>Odkazy Δ {result.summary.linkCountDelta}</li>
            <li>Stránky Δ {result.summary.pageCountDelta}</li>
          </ul>
          {result.changes.length > 0 && (
            <div className="max-h-40 overflow-y-auto space-y-1 border-t border-border pt-2">
              {result.changes.slice(0, 25).map((c, i) => (
                <div key={`${c.path}-${i}`} className="mono text-[10px] text-fg-subtle break-all">
